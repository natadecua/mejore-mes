...
export default function DesignI() {
  ...
  // Initialize active item once BOMS is available
  useEffect(() => {
    if (BOMS.length > 0 && !activeItem) {
      setActiveItem(BOMS[0].children?.[1]?.children?.[0] || BOMS[0])
    }
  }, [])
  ...
  if (!activeItem) {
    return <div className="min-h-screen bg-black flex items-center justify-center text-white">Loading Hierarchy...</div>
  }
      ...
      <aside className="w-96 bg-[#0f1116] border border-slate-800/50 rounded-3xl flex flex-col shadow-2xl overflow-hidden shrink-0">
        <div className="p-6 bg-slate-900/50 border-b border-slate-800">
           ...
           <div className="bg-black/40 border border-slate-800 p-4 rounded-2xl">
              ...
              <h3 className="text-white font-black text-lg leading-tight uppercase tracking-tight">{BOMS[0]?.name || 'Loading...'}</h3>
           </div>
        </div>
        ...
        <div className="p-4 bg-black/40 flex items-center justify-between border-t border-slate-800">
           <button className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white text-[10px] font-black py-3 px-6 rounded-xl transition-all w-full justify-center shadow-lg active:scale-95">
      ...
      <main className="flex-1 bg-[#0f1116] border border-slate-800/50 rounded-3xl flex flex-col shadow-2xl overflow-hidden min-w-0">
        ...
        <div className="h-14 bg-slate-900/80 border-b border-slate-800 px-6 flex items-center justify-between shrink-0">
           <div className="flex items-center gap-4">
             ...
             <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest whitespace-nowrap hidden md:block">Op: <span className="text-white">Nathan H.</span></span>
           </div>
        ...
        {/* Focus Area */}
        <div className="flex-1 p-6 md:p-8 overflow-auto flex flex-col gap-6">
           <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 shrink-0">
              ...
              <div className="text-right flex flex-col items-end shrink-0">
                <div className="w-48 h-2 bg-slate-800 rounded-full overflow-hidden mb-2 shadow-inner">
                  <div className="h-full bg-amber-600 transition-all duration-500" style={{ width: `${activeItem.progress || 0}%` }}></div>
                </div>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Part Completion: <span className="text-white">{activeItem.progress || 0}%</span></p>
              </div>
           </div>
           ...
           <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 flex-1">
              ...
              <div className="xl:col-span-7 flex flex-col gap-4 min-h-[350px]">
                 <div className="bg-black/60 border border-slate-800 rounded-[2.5rem] p-10 flex-1 flex flex-col justify-between items-center text-center shadow-inner relative overflow-hidden">
                    ...
                    <div className="space-y-2 relative z-10">
                       ...
                       <div className="text-7xl md:text-[9rem] font-black text-white leading-none tabular-nums tracking-tighter">
                         {formatTime(timer)}
                       </div>
                    </div>
                    ...
                    <div className="w-full grid grid-cols-2 gap-4 md:gap-6 relative z-10">
                       {workState !== 'running' ? (
                         <button 
                           ...
                           className="bg-amber-600 hover:bg-amber-500 text-black py-8 md:py-10 rounded-3xl font-black text-lg md:text-2xl uppercase shadow-2xl shadow-amber-600/20 border-b-8 border-amber-800 active:border-b-0 active:translate-y-2 transition-all"
                         >
                           Start Job
                         </button>
                       ) : (
                         <button 
                           ...
                           className="bg-slate-800 hover:bg-slate-700 text-white py-8 md:py-10 rounded-3xl font-black text-lg md:text-2xl uppercase border-b-8 border-slate-900 active:border-b-0 active:translate-y-2 transition-all"
                         >
                           Pause
                         </button>
                       )}
                       
                       <button className="bg-white hover:bg-zinc-100 text-black py-8 md:py-10 rounded-3xl font-black text-lg md:text-2xl uppercase border-b-8 border-slate-300 active:border-b-0 active:translate-y-2 shadow-xl shadow-white/5 transition-all">
                         Complete
                       </button>
                    </div>
                 </div>
              ...
              <div className="xl:col-span-5 flex flex-col gap-4">
                 <section className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6 flex flex-col gap-4">
                    ...
                    <div className="space-y-4">
                       <div className="flex justify-between border-b border-slate-800/50 pb-2">
                         <span className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Machining</span>
                         <span className="text-white text-xs font-black">2mm Runner Gap</span>
                       </div>
                       <div className="flex justify-between border-b border-slate-800/50 pb-2">
                         <span className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Finishing</span>
                 ...
                 <div className="grid grid-cols-2 gap-4 flex-1">
                    ...
                    <button className="bg-red-500/10 border border-red-500/30 hover:bg-red-500/20 text-red-500 rounded-3xl p-6 flex flex-col items-center justify-center gap-3 transition-all active:scale-95 group">
                       ...
                       <span className="font-black text-[9px] uppercase tracking-widest text-center">Flag Incident</span>
                    </button>
                 </div>
              </div>
           </div>
